...
x = layers.Dense(128, activation='relu')(inputs)  # First hidden layer
x = layers.BatchNormalization()(x)  # Batch normalization
x = layers.Dropout(0.4)(x)  # Dropout for regularization

# Skip connection: Connect the input directly to the next layer
skip1 = layers.Dense(128, activation='relu')(inputs)  # Skip connection layer
x = layers.Add()([x, skip1])  # Add skip connection

x = layers.Dense(64, activation='relu')(x)  # Second hidden layer
x = layers.BatchNormalization()(x)  # Batch normalization
x = layers.Dropout(0.4)(x)  # Dropout for regularization

# Third hidden layer with another skip connection
skip2 = layers.Dense(64, activation='relu')(skip1)  # Skip connection layer
x = layers.Add()([x, skip2])  # Add skip connection
...
model.save('sanction_predictor_with_all_features.h5')
